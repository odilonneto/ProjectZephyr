...
MEMORY
    ...
    CCM ( rw ) : ORIGIN = (268435456), LENGTH = (65536) SDRAM2 ( rw ) : ORIGIN = (3489660928), LENGTH = (8388608)
...
.intList :
{
 ...
 KEEP(*(.gnu.attributes))
    ...
    CCM (NOLOAD) : { __CCM_start = .; KEEP(*(CCM)) KEEP(*(CCM.*)) __CCM_end = .; } > CCM __CCM_size = __CCM_end - __CCM_start; __CCM_load_start = LOADADDR(CCM); SDRAM2 (NOLOAD) : { __SDRAM2_start = .; KEEP(*(SDRAM2)) KEEP(*(SDRAM2.*)) __SDRAM2_end = .; } > SDRAM2 __SDRAM2_size = __SDRAM2_end - __SDRAM2_start; __SDRAM2_load_start = LOADADDR(SDRAM2);
.last_section :
{
  KEEP(*(.last_section))
} > FLASH
